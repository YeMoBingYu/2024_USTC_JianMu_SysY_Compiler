...
static std::vector<std::shared_ptr<ConstantArray>> cached_array;
...
{
    auto ptr = new ConstantArray(ty, val);
    auto ptr_shared = std::shared_ptr<ConstantArray>(ptr);
    cached_array.push_back(ptr_shared);
    return ptr_shared.get();
}
...
std::string ConstantArray::print()
{
    ...
    const_ir += " ";
    if (this->get_size_of_array() > 0 && !dynamic_cast<ConstantZero *>(get_element_value(0)))
    {
        const_ir += "[";
